<template>
	<div class="v-table-dynamic" :class="{ 'is-fit-height': fitToHeight }" ref="tableWrapper">
		<div v-if="tableData && tableData.rows && tableData.rows.length > 0" :style="{ minWidth: minWidth + 'px', maxWidth: maxWidth + 'px' }">
			<!-- Table Tools -->
			...
			>
				...
				<div class="v-table-header-wrap" ref="tableHeader">
					...
					>
						<div
							...
							:style="{
								backgroundColor: isHighlighted(0, NaN) ? highlightedColor : 'transparent',
							}"
						>
							<div class="table-check-all flex-c-c" :class="{ 'is-checked': tableData.rows[0].checked }" @click.stop="onCheckAll(tableData.rows[0])">
						...
						>
							<span v-if="!fixedColumn.includes(j)" class="table-cell-inner flex-c-s">
								<span
									...
									:style="{
										...
										...getStyleCustomized(0, j),
									}"
								>
									<div v-if="useHTMLAsHeaderName" v-html="tableCell.data"></div>
								...
								<span v-if="filterConfig[j]" class="table-filter flex-c-c" :style="{ height: headerHeight + 'px' }">
									<filter-panel
										:content="filterConfig[j].content"
										:lang="lang"
										@filter="
											(checked) => {
												onFilter(j, checked, filterConfig[j]);
											}
										"
										@reset="clearFilter(j)"
									>
										<i
											...
											:style="{
												color: activatedFilter[j] ? activedColor : '#C0C4CC',
											}"
										>
										</i>
									</filter-panel>
				...
				<div class="v-table-body" :style="{ height: bodyHeight }">
					<vue-scrollbar x-bar-display="hidden" :y-bar-display="scrollbarDisplay" :size="scrollbarSize" :scrollbarColor="scrollbarColor" :scrollbarHoverColor="scrollbarHoverColor" :border-radius="scrollbarBorderRadius" :step="scrollStep" :delay="scrollDelay" @scroll-x="onScrollX" @scroll-y="onScrollY" @size="onSize" ref="scrollbar">
						<div v-for="(tableRow, i) in tableData.activatedRows" :key="i" :style="{ minWidth: getRowMinWidth() }">
							...
							>
								<div
									...
									:class="{ 'v-show-border': tableBorder }"
									:style="{
										backgroundColor: isHighlighted(tableRow.index, NaN) ? highlightedColor : 'transparent',
									}"
								>
									<div class="table-check-row flex-c-c" :class="{ 'is-checked': tableRow.checked }" @click.stop="onCheckRow(tableRow, tableRow.index)">
								...
								>
									<slot
										...
										:name="'column-' + j"
										v-bind:props="{
											cellData: tableCell.data,
											rowData: tableRow.cells,
											row: tableRow.index,
											column: j,
										}"
									>
										<span
											...
											:style="{
												...
												wordWrap: wordWrap,
												textOverflow: textOverflow,
												...getStyleCustomized(tableRow.index, j),
											}"
				...
				<!-- Table Fixed -->
				<div class="v-table-fixed" :class="{ 'is-show-shadow': scrollx !== 'left' }" v-if="fixedWidth > 0" :style="{ width: fixedWidth + 'px' }">
					<!-- Fixed Header -->
					<div
						v-if="headerInfirstRow"
						class="v-table-row flex-c is-header"
				...
				<div class="v-table-scrollbar">
					<horizontal-scrollbar v-if="bodyWidth && bodyViewerWidth" :x-bar-display="scrollbarDisplay" :color="scrollbarColor" :hoverColor="scrollbarHoverColor" :size="scrollbarSize" :border-radius="scrollbarBorderRadius" :viewer-width="bodyViewerWidth" :wrapper-width="bodyWidth" :scrolling="hMovement" @change-position="onChangePosition" ref="hscroll">
					</horizontal-scrollbar>
				</div>
			</div>
			<!-- Table Pagination -->
			<div class="table-pagination" v-if="pagination" ref="paginationWrapper">
				<vue-pagination :page-size="pageSize" :page-sizes="pageSizes" :total="totalPages" :show-total="showTotal" :paper-count="paperCount" :disabled="!pagination" :lang="lang" @current-page="onPageChange" @size="onPageSizeChange" ref="tablePagination"> </vue-pagination>
			</div>
		</div>
	...
	export default {
		...
		methods: {
			...
			isEditable(rowIndex, columnIndex) {
				...
				if (Array.isArray(this.editConfig.row) && (this.editConfig.row.includes(rowIndex) || this.editConfig.row.includes(rowIndex - this.sourceData.length))) {
					return true;
				}

				if (Array.isArray(this.editConfig.column) && (this.editConfig.column.includes(columnIndex) || this.editConfig.column.includes(columnIndex - this.sourceData[0].length))) {
					return true;
				}
			...
			isHighlighted(rowIndex, columnIndex) {
				...
				if (Array.isArray(this.highlightConfig.row) && (this.highlightConfig.row.includes(rowIndex) || this.highlightConfig.row.includes(rowIndex - this.sourceData.length))) {
					return true;
				}

				if (Array.isArray(this.highlightConfig.column) && (this.highlightConfig.column.includes(columnIndex) || this.highlightConfig.column.includes(columnIndex - this.sourceData[0].length))) {
					return true;
				}
				...
				if (Array.isArray(this.styleConfig.row)) {
					for (let i = 0; i < this.styleConfig.row.length; i++) {
						...
						if (typeof item.styles === "object") {
							if (item.scope === "all") {
								return item.styles;
							} else if (Array.isArray(item.scope) && (item.scope.includes(rowIndex) || item.scope.includes(rowIndex - this.sourceData.length))) {
								return item.styles;
							}
						}
					}
				...
				if (Array.isArray(this.styleConfig.column)) {
					for (let i = 0; i < this.styleConfig.column.length; i++) {
						...
						if (typeof item.styles === "object") {
							if (item.scope === "all") {
								return item.styles;
							} else if (Array.isArray(item.scope) && (item.scope.includes(columnIndex) || item.scope.includes(columnIndex - this.sourceData[0].length))) {
								return item.styles;
							}
						}
					}
			...
			onCheckAll(tableRow) {
				...
				this.$emit("selection-change", this.getCheckedRowDatas(true), this.getCheckedRowIndexs(true), this.getCheckedRowNum(true));
			},
			/**
			 * @function 勾选单个Row
				...
				this.tableData.rows.forEach((row) => {
					if (row && row.cells) {
						...
						let matched = row.cells.some((cell, index) => {
							...
							const transformItem = Array.isArray(transform) ? transform.find((item) => item && item.column === index && typeof item.method === "function") || null : null;
...
<style lang="scss" scoped>
	...
	.v-table-fixed.is-show-shadow {
		-webkit-box-shadow: 1px 0px 6px rgba(0, 0, 0, 0.12);
